...
function updateUI() {
    ...
    rollBtn.textContent = `Roll Dice (${game.diceLeft} dice left)`;
    ...
    nextTurnBtn.classList.toggle('hidden', game.diceLeft === 0);
...
function renderDice() {
    ...
    const isFirstRoll = game.firstTurn === true;
    ...
    for (const die of game.diceSet.dice) {
        ...
        dieEl.addEventListener('click', () => {
            ...
            die.toggleHold();
            if (die.isHeld) game.dieChosen++; //tracks if you can reroll or not
            else game.dieChosen--;
